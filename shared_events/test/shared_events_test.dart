...
void main() {
  ...
  group('PlayerAttributes', () {
    test('defaults describe a fresh level-1 character', () {
    ...
    test('fromMap falls back to defaults on missing keys and tolerates '
        'numeric strings', () {
    ...
    test('copyWith replaces fields and clamps hp/stamina', () {
      ...
      expect(base.copyWith(level: 4, xp: 5), const PlayerAttributes(level: 4, xp: 5, hp: 50, stamina: 30));
    });
    ...
    test('addXp levels up every 100 XP keeping the remainder', () {
      ...
      expect(
        ...
        const PlayerAttributes(level: 2, xp: 0),
      );
      expect(
        const PlayerAttributes().addXp(250),
        const PlayerAttributes(level: 3, xp: 50),
      );
      expect(
        const PlayerAttributes(level: 5, xp: 90).addXp(20),
        const PlayerAttributes(level: 6, xp: 10),
      );
