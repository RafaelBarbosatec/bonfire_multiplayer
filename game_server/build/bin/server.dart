...
import '../routes/characters.dart' as characters;
...
Handler buildHandler() {
  final pipeline = const Pipeline();
  final router = Router()
    ..all('/ws', (context) => ws.onRequest(context,))..all('/players_game', (context) => players_game.onRequest(context,))..all('/players', (context) => players.onRequest(context,))..all('/maps', (context) => maps.onRequest(context,))..all('/', (context) => index.onRequest(context,))..all('/characters', (context) => characters.onRequest(context,));
  return pipeline.addHandler(router);
}
